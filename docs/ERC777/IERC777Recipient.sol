...
//
// 以下にIERC777Recipientと`tokensReceived`が使用される場面を示します. 先にERC777とERC1820 Registryのファイル冒頭の説明に目を通してください.
// 登場人物: ERC777のholderであるH. HのoperatorであるO.
// 前提: Hは, 自身にトークンが転送される際に
//      通知または何らかのアクション(特定の条件下では転送のトランザクションをキャンセルさせるなど)を起こしたいと考えているとします.
//      Hは, その処理を実装したコントラクト(つまりIERC777Recipientを実装し`tokensReceived`関数に処理を記述したコントラクト)を用意し, ERC1820 Registryに登録しています.
// 1. OがHにトークンを送信(または発行)しようとERC777のsend関数を呼び出します.
// 2. send関数内では, Hの残高を変更後にHがIERC777Recipientを実装したコントラクトを登録しているかERC1820 Registryに問い合わせます.
