...
よって, 毎回のトークン転送において, トークン保有者とトークン受信者の両側で(必要があれば)トランザクション内容のチェックが行われるため, トークンの誤送信を最小限に防ぐことができます.
...
アカウント間に明確な関係性を定義することは, 各アカウントの役割をわかりやすくし, トークンコントラクトの機能の利用者の混乱を抑えます.

例えば, ERC777 において, ホルダーがトークンを送信するときには`send`関数を使用し, オペレータがトークンを送信するときには`operatorSend`関数を使用します.  
アカウントの役割が明確化されていることで, ホルダー・オペレータはそれぞれどの関数を使用してトークンを送信すれば良いのか一目瞭然です.
