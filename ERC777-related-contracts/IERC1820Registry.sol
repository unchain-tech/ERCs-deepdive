...
// このファイルは`ERC1820`のインタフェースを定義しています.
// `ERC1820`は, 任意のアカウント(レビュラーアカウント/コントラクト)が「あるインタフェースとそのインタフェースを実装したコントラクト」を登録することを可能にします.
// コントラクトが`ERC1820`に自身を登録することで, 特定のインタフェースを実装していることを判別できるようにすることも可能ですし, 
...
//
// `ERC1820`コントラクトは, ブロックチェーン内にある上記の存在の関係性を管理する台帳なので, ブロックチェーン内に1つだけ存在します.
