...
// このファイルはERC1820 Registryのインタフェースを定義しています.
// ERC1820 Registryは, 任意のアカウント(レビュラーアカウント/コントラクト)が「あるインタフェースとそのインタフェースを実装したコントラクト」を登録することを可能にします.
// コントラクトがERC1820 Registryに自身を登録することで, 特定のインタフェースを実装していることを判別できるようにすることも可能ですし, 
// レギュラーアカウントが「自身に代わってあるインタフェースを実装したコントラクト」を登録することも可能です.
//
// このコントラクトの理解には, 以下の存在の認識が必要です.
// ・コントラクトを登録するアカウント(レギュラーアカウント/コントラクト)
// ・登録されるインタフェース
// ・登録されるコントラクトcode 
// ・manager: コントラクトの登録作業が許されるアカウント
// 　デフォルトでは, 「全てのアカウントは自分自身のmanagerでもあります」=「全てのアドレスはコントラクトの登録が可能です」.
//
// ERC1820 Registryコントラクトは, ブロックチェーン内にある上記の存在の関係性を管理する台帳なので, ブロックチェーン内に1つだけ存在します.
