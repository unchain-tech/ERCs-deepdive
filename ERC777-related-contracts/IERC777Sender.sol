...
// 先にERC777と`IERC1820`について, ファイル冒頭の説明などで概要を理解してください.
// 登場人物: ERC777のholderであるH. HのoperatorであるO.
// 前提: Hは, 自身のトークンが転送される際に
//      何らかのアクション(記録や特定の条件下では転送のトランザクションをキャンセルさせるなど)を起こしたいと考えているとします.
//      Hは, その処理を実装したコントラクト(つまりIERC777Senderを実装し`tokensToSend`関数に処理を記述したコントラクト)を用意し, `IERC1820`に登録しています.
// 1. OがHのトークンをどこかに送信(または焼却)しようとERC777のsend関数を呼び出します.
// 2. send関数内では, Hの残高を変更する前にHがIERC777Senderを実装したコントラクトを登録しているか`IERC1820`に問い合わせます.
