...
// `IERC777Recipient`は`tokensReceived`という関数の実装を定義するインタフェースです.
// `tokensReceived`はあるアカウントに対して「トークンが転送される」=「トークンが増加する」アクションが起きた際に, 
// そのことをアカウント(トークン受信者)に通知する役目を担います.
//
// 以下に`IERC777Recipient`と`tokensReceived`が使用される場面を示します. 先に`ERC777`と`IERC1820`のファイル冒頭の説明に目を通してください.
...
//      Hは, その処理を実装したコントラクト(つまり`IERC777Recipient`を実装し`tokensReceived`関数に処理を記述したコントラクト)を用意し, `IERC1820`に登録しています.
// 1. OがHにトークンを送信(または発行)しようと`ERC777`のsend関数を呼び出します.
// 2. send関数内では, Hの残高を変更後にHが`IERC777Recipient`を実装したコントラクトを登録しているか`IERC1820`に問い合わせます.
